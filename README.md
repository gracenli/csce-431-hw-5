...
----
Clone **your fork** of this repo to your development environment (**do
not clone** this reference repo or you'll be unable to push your
changes);

`git clone https://github.tamu.edu/YOUR_NETID/hw-5-acceptance-unit-test-cycle`

Once you have the clone of the repo:

1. Change into the rottenpotatoes directory: `cd hw-5-acceptance-unit-test-cycle/rottenpotatoes`  

2. Run `bundle config set --local without 'production'` and then `bundle install` to make sure all gems are properly installed.  

**NOTE:** If Bundler complains that the wrong
...
spec`.  (Note--*not* `rake rspec` as you might think. Sorry.)  There are already some RSpec tests written and you should expect them to fail right now.
